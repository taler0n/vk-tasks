...
def main():
    ...
    for user_id in user_ids:
        ...
        career = get_career(api, tools, user_id)
        schools = get_schools(api, tools, user_id)
        universities = get_universities(api, tools, user_id)
        print("\tworks")
        ...
        for group_id in groups:
            ...
            if regex_handler.check_group(
                ...
                indexed_groups[group_id] = {"name": group_info["name"], "tag": "EDUCATION"}
                break
            if (group_id in found_groups_career
                    or regex_handler.check_group(group_info, found_info_career, keywords_career)):
                indexed_groups[group_id] = {"name": group_info["name"], "tag": "WORK"}
...
def get_schools(api, tools, user_id):
    schools_name = {}
    schools = api.users.get(user_ids=user_id, fields="schools")

    for school in schools[0]["schools"]:
        if school.get("id") is not None:
            school_name = school["id"]
        else:
            school_name = school["id"]
        schools_name[school_name] = dict.fromkeys(["info", "dt_start", "dt_end", "type"])
        school_info = {}
        ...
        schools_name[school_name]["info"] = school_info
        if school.get("year_from") is not None:
            schools_name[school_name]["dt_start"] = school["year_from"]
        if school.get("year_to") is not None:
            schools_name[school_name]["dt_end"] = school["year_to"]
        elif school.get("year_graduated") is not None:
            schools_name[school_name]["dt_end"] = school["year_graduated"]
        if school.get("type_str") is not None:
            schools_name[school_name]["type"] = school["type_str"]
        else:
            schools_name[school_name]["type"] = "SCHOOL"

    return schools_name


def get_universities(api, tools, user_id):
    universities_name = {}
    universities = api.users.get(user_ids=user_id, fields="universities")

    for university in universities[0]["universities"]:
        if university.get("id") is not None:
            school_name = university["id"]
        else:
            school_name = university["id"]
        universities_name[school_name] = dict.fromkeys(["info", "dt_start", "dt_end", "type"])
        university_info = {}
        ...
        universities_name[school_name]["info"] = university_info
        if university.get("graduation") is not None:
            universities_name[school_name]["dt_end"] = university["graduation"]
        universities_name[school_name]["type"] = "UNIVERSITY"

    return universities_name


def get_career(api, tools, user_id):
    career_name = {}
    user_info = api.users.get(user_ids=user_id, fields="career")

    for career in user_info[0]["career"]:
        if career.get("group_id") is not None:
            work_name = career["group_id"]
        else:
            work_name = career["company"]
        career_name[work_name] = dict.fromkeys(["info", "dt_start", "dt_end"])
        career_info = {}
        ...
        career_name[work_name]["info"] = career_info
        if career.get("from") is not None:
            career_name[work_name]["dt_start"] = career["from"]
        if career.get("until") is not None:
            career_name[work_name]["dt_end"] = career["until"]
